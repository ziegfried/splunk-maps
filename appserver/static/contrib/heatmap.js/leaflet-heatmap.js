...
var HeatmapOverlay = L.Layer.extend({
    ...
    _destroyHeatmap: function() {
        this._heatmap = null;
        while (this._el.childNodes.length) {
            this._el.removeChild(this._el.childNodes[0]);
        }
    },
    
    _resetSize: function(e) {
        this._updateSize(e.newSize);
        this._draw(true);
    },

    onAdd: function(map) {
        this._updateSize(map.getSize());
        map.on('resize', this._resetSize, this);
    ...
    _draw: function(createHeatmap) {
        ...
        this._el.style[L.DomUtil.TRANSFORM] = 'translate(' + -Math.round(point.x) + 'px,' + -Math.round(point.y) + 'px)';
    ...
    clear: function() {
        this._data = [];
        if (this._heatmap) {
            this._heatmap.setData({ data: this._data });
        }
    },
